...
public class KerberosAppActivity extends Activity implements
		gsswrapperConstants {
	...
	private int startClient() throws Exception {
		...
		maj_status = gsswrapper.gss_add_oid_set_member(min_status, neg_mech,
				neg_mech_set);
		...
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "calling gss_oid_to_str", min_status,
					maj_status);
			return -1;
		}
		...
		if (ret == 0) {
			...
			maj_status = gsswrapper.gss_delete_sec_context(min_status, context,
					output_token);

			if (maj_status != GSS_S_COMPLETE) {
				Util.displayError(tv, "deleting security context", min_status,
						maj_status);
				ret = -1;
			}
	...
	private int Authenticate(Socket clientSocket, InputStream serverIn,
			OutputStream serverOut, String inClientName, String inServiceName,
		...
		/*
		 * Client picks client principal it wants to use. Only done if we know
		 * what client principal will get the service principal we need.
		 */
		if (inClientName != null) {
			...
			maj_status = gsswrapper.gss_acquire_cred(min_status, clientName,
					...
					clientCredentials, null, time_rec);
			if (maj_status != GSS_S_COMPLETE) {
			...
			if (neg_mech_set != GSS_C_NO_OID_SET) {
				maj_status = gsswrapper.gss_set_neg_mechs(min_status,
						clientCredentials, neg_mech_set);
		...
		maj_status = gsswrapper.gss_inquire_cred(min_status, clientCredentials,
				name, lifetime, cred_usage, temp_mech_set);
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "gss_inquire_cred(temp_mech_set)",
					min_status, maj_status);
			return -1;
		}
		maj_status = gsswrapper.gss_release_oid_set(min_status, temp_mech_set);
		if (maj_status != GSS_S_COMPLETE) {
		...
		maj_status = gsswrapper.gss_duplicate_name(min_status, clientName,
				clientName_dup);
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "duplicating client name", min_status,
					maj_status);
			return -1;
		}
		...
		maj_status = gsswrapper.gss_canonicalize_name(min_status, clientName,
				gss_mech_krb5, clientCanonicalized);
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "canonicalizing client name", min_status,
					maj_status);
			return -1;
		}
		...
		maj_status = gsswrapper.gss_export_name(min_status,
				clientCanonicalized, clientName_export);
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "exporting client name", min_status,
					maj_status);
			return -1;
		}
		...
		/*
		 * Client picks the service principal it will try to use to connect to
		 * the server. The server principal is given at the top of this file.
		 */
		gss_buffer_desc nameBuffer = new gss_buffer_desc();
		...
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "gss_import_name(inServiceName)", min_status,
					maj_status);
			return -1;
		}

		/*
		 * The main authentication loop. Because GSS is a multimechanism API, we
		 * need to loop calling gss_init_sec_context - passing in the
		...
		while (maj_status != GSS_S_COMPLETE) {
			...
			long requestedFlags = (GSS_C_MUTUAL_FLAG ^ GSS_C_REPLAY_FLAG
			...
			maj_status = gsswrapper.gss_init_sec_context(min_status,
					clientCredentials, context_tmp, serverName, GSS_C_NO_OID,
					requestedFlags, GSS_C_INDEFINITE,
					GSS_C_NO_CHANNEL_BINDINGS, inputToken, actual_mech_type,
					outputToken, actualFlags, time_rec);

			if (outputToken.getLength() > 0) {
				/*
				 * Send the output token to the server (even on error), using a
				 * Java byte[]
				 */
				byte[] temp_token = new byte[(int) outputToken.getLength()];
				temp_token = gsswrapper.getDescArray(outputToken);
				tv.append("Generated Token Length = " + temp_token.length
						+ "\n");
			...
			if (err == 0) {
				if (maj_status == GSS_S_CONTINUE_NEEDED) {
					...
					Util.displayError(tv, "gss_init_sec_context", min_status,
							maj_status);
					return -1;
				}
			}
		...
		maj_status = gsswrapper.gss_compare_name(min_status, clientName,
				serverName, ret);
		if (ret[0] == 1) {
	...
	private int Communicate(Socket clientSocket, InputStream serverIn,
		...
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "wrapping message, gss_wrap", min_status,
					maj_status);
			return -1;
		} else if (state[0] == 0) {
		...
		maj_status = gsswrapper.gss_verify_mic(min_status, context, in_buf,
				out_buf, qop_state);
	...
	private int AltCommunicate(Socket clientSocket, InputStream serverIn,
			OutputStream serverOut) {
		TextView tv = (TextView) findViewById(R.id.textView);
		...
		maj_status = gsswrapper.gss_export_sec_context(min_status, context,
				context_token);
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "exporting security context", min_status,
					maj_status);
			return -1;
		} else {
		...
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "importing security context", min_status,
					maj_status);
			return -1;
		} else {
		...
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "wrapping message, gss_seal", min_status,
					maj_status);
			return -1;
		} else if (state[0] == 0) {
			tv.append("Warning!  Message not encrypted.\n");
		...
		/* Verify signature block */
		maj_status = gsswrapper.gss_verify(min_status, context, in_buf,
				out_buf, qop_state);
	...
	private int PrintContextInfo() {
		...
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "checking for valid context", min_status,
					maj_status);
			return -1;
		}

		/* Get context source name */
		maj_status = gsswrapper.gss_display_name(min_status, src_name, sname,
				name_type);
		if (maj_status != GSS_S_COMPLETE) {
		...
		maj_status = gsswrapper.gss_display_name(min_status, targ_name, tname,
				name_type);
		if (maj_status != GSS_S_COMPLETE) {
		...
		tv.append("---------------------- Context Information -----------"
				+ "-----------\n");
		tv.append("Context is valid for " + time_rec[0] + " seconds\n");
		tv.append(sname.getValue() + " to " + tname.getValue() + "\n");
		tv.append("Lifetime: " + lifetime[0] + " seconds\n");
		tv.append("Initiated: "
				+ ((is_local[0] == 1) ? "locally\n" : "remotely\n"));
		tv.append("Status: " + ((is_open[0] == 1) ? "open\n" : "closed\n"));
		...
		maj_status = gsswrapper.gss_oid_to_str(min_status, name_type, oid_name);
		if (maj_status != GSS_S_COMPLETE) {
		...
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "Inquiring mechanism attributes", min_status,
					maj_status);
			return -1;
		}
		...
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "Inquiring mech names", min_status,
					maj_status);
			return -1;
		}

		maj_status = gsswrapper.gss_oid_to_str(min_status, mechanism, oid_name);
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "Converting oid->string", min_status,
					maj_status);
			return -1;
		}

		tv.append("Mechanism " + oid_name.getValue() + " supports "
				+ mech_names.getCount() + " names\n");
		...
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "Inquiring SASL name", min_status, maj_status);
			return -1;
		}
		...
		maj_status = gsswrapper.gss_inquire_mech_for_saslname(min_status,
				sasl_mech_name, oid);
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "Inquiring mechs for SASL name", min_status,
					maj_status);
			return -1;
		}
		...
		/*
		 * Determine largest message that can be wrapped, assuming max output
		 * token size of 100 (just for testing)
		 */
		long[] max_size = { 0 };
		maj_status = gsswrapper.gss_wrap_size_limit(min_status, context, 1,
				GSS_C_QOP_DEFAULT, 100, max_size);
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "determining largest wrapped message size",
					min_status, maj_status);
			return -1;
		} else {
			tv.append("Largest message size able to be wrapped: " + max_size[0]
					+ "\n");
		}
	...
	private int MiscFunctionTests() {
		...
		/*
		 * FUNCTION: gss_inquire_cred_by_mech Get info about default cred for
		 * default security mech.
		 */
		maj_status = gsswrapper.gss_inquire_cred_by_mech(min_status,
				cred_handle, mech_type, name, init_lifetime, accept_lifetime,
				cred_usage);
		if (maj_status != GSS_S_COMPLETE) {
		...
		maj_status = gsswrapper.gss_pseudo_random(min_status, context, 0,
				prf_in, 19, prf_out);
		if (maj_status != GSS_S_COMPLETE) {
		...
		maj_status = gsswrapper.gss_indicate_mechs_by_attrs(min_status,
				...
				mechs);
		if (maj_status != GSS_S_COMPLETE) {
			Util.displayError(tv, "gss_indicate_mechs_by_attrs", min_status,
					maj_status);
			return -1;
		}
